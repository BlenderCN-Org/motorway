...
static inline std::map<fnStringHash_t, CustomVariableType>& ENV_VAR_TYPES()
{
    static std::map<fnStringHash_t, CustomVariableType> internalMap = {};
    return internalMap;
}

inline std::map<fnStringHash_t, Variable>& EnvironmentVariables::getEnvironmentVariableMap()
{
    static std::map<fnStringHash_t, Variable> internalMap = {};
    return internalMap;
}

void EnvironmentVariables::readValue( Variable& variable, const fnString_t& inputValue )
{
    ...
    case FLAN_STRING_HASH( "double" ):
        *( double* )variable.Value = std::stod( inputValue );
        break;
    case FLAN_STRING_HASH( "float" ):
        *( float* )variable.Value = std::stof( inputValue );
        break;
    case FLAN_STRING_HASH( "long long" ):
    case FLAN_STRING_HASH( "int64_t" ):
        *( int64_t* )variable.Value = std::stoll( inputValue );
        break;
    case FLAN_STRING_HASH( "unsigned long long" ):
    case FLAN_STRING_HASH( "uint64_t" ):
        *( uint64_t* )variable.Value = std::stoull( inputValue );
        break;
    case FLAN_STRING_HASH( "int" ):
    case FLAN_STRING_HASH( "int32_t" ):
        *( int32_t* )variable.Value = std::stoi( inputValue );
        break;
    case FLAN_STRING_HASH( "unsigned int" ):
    case FLAN_STRING_HASH( "uint32_t" ):
        *( uint32_t* )variable.Value = std::stoi( inputValue );
        break;
        // TODO Not safe
    case FLAN_STRING_HASH( "short" ):
    case FLAN_STRING_HASH( "int16_t" ):
        *( int16_t* )variable.Value = std::stoi( inputValue );
        break;
    case FLAN_STRING_HASH( "unsigned short" ):
    case FLAN_STRING_HASH( "uint16_t" ):
        *( uint16_t* )variable.Value = std::stoi( inputValue );
        break;
    case FLAN_STRING_HASH( "bool" ):
        *( bool* )variable.Value = ( inputValue == FLAN_STRING( "True" ) );
        break;
    default: {
...
int EnvironmentVariables::registerCustomType( const fnStringHash_t typeHashcode, fnToStringEnvType_t toStringFunc, fnFromStringEnvType_t fromStringFunc )
{
    ...
    for ( auto& var : getEnvironmentVariableMap() ) {
        ...
        default:
            ...
            if ( it != ENV_VAR_TYPES().end() ) {
                ...
                FLAN_COUT << "Unknown type (with hashcode: 0x" << std::hex << varInfos.Type << ")" << std::endl;
                continue;
            }
            break;
        }
        ...
        if ( keyValueSeparator != fnString_t::npos ) {
            ...
            if ( !dictionaryValue.empty() ) {
                ...
                if ( variablesIterator != getEnvironmentVariableMap().end() ) {
                    ...
                    FLAN_CERR << "Unknown environment variable '" << dictionaryKey << "' (either spelling mistake or deprecated)" << std::endl;
                }
            }
        }
    }
