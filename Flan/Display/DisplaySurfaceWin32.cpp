...
{
    FLAN_CLOG << "Creating display surface (WIN32)" << std::endl;

    auto instance = GetModuleHandle( NULL );
    auto className = FLAN_STRING( "Flan GameEngine Window" );
...
#define IDI_ICON1                       101
    ...
    NativeDisplaySurface* displaySurface = new NativeDisplaySurface();
...
{
    SetWindowText( surface->Handle, caption.c_str() );
}
...
void flan::core::ToggleFullscreenImpl( NativeDisplaySurface* surface )
{
    ...
    if ( ChangeDisplaySettings( &devMode, CDS_FULLSCREEN ) != DISP_CHANGE_SUCCESSFUL ) {
        FLAN_CERR << "Failed to toggle fullscreen (ChangeDisplaySettings failed)" << std::endl;
    }
}
