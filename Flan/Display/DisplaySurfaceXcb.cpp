...
inline xcb_intern_atom_cookie_t GetCookieForAtom( xcb_connection_t* connection, const fnString_t& stateName )
{
...
xcb_atom_t GetReplyAtomFromCookie( xcb_connection_t* connection, xcb_intern_atom_cookie_t cookie )
{
    ...
    if ( error != nullptr ) {
        FLAN_CERR << "Internal failure! (error code:" << error->error_code << ")" << std::endl;
    }

    return reply->atom;
}

NativeDisplaySurface* flan::core::CreateDisplaySurfaceImpl( const uint32_t surfaceWidth, const uint32_t surfaceHeight )
{
    FLAN_CLOG << "Creating display surface (XCB)" << std::endl;

    xcb_connection_t* connection = xcb_connect( nullptr, nullptr );
    if ( connection == nullptr ) {
        FLAN_CERR << "Failed to connect to Xorg server (xcb_connect returned nullptr)" << std::endl;
        return nullptr;
    }

    FLAN_CLOG << "\tConnected to X11 server" << std::endl;
    ...
    FLAN_CLOG << "\tCreated and mapped window to screen successfully!" << std::endl;
    ...
    int w, h, comp;
    auto image = stbi_load( "flan_icon.png", &w, &h, &comp, STBI_rgb_alpha );
    if ( image != nullptr ) {
...
void flan::core::SetDisplaySurfaceCaptionImpl( NativeDisplaySurface* surface, const fnString_t& caption )
{
...
void flan::core::ToggleFullscreenImpl( NativeDisplaySurface* surface )
{
...
void flan::core::ToggleBorderlessImpl( NativeDisplaySurface* surface )
{

}

bool flan::core::GetShouldQuitFlagImpl( NativeDisplaySurface* surface )
{
    return ( surface->Flags.ShouldQuit == 1 );
}

void flan::core::SetMousePositionImpl(  NativeDisplaySurface* surface, const float surfaceNormalizedX, const float surfaceNormalizedY )
{
