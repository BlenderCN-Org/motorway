...
#include <Framework/Cameras/FirstPersonCamera.h>
#include "SceneNode.h"
#include <Graphics/DrawCommandBuilder.h>

struct FirstPersonCameraSceneNode : public SceneNode
{
    virtual FirstPersonCameraSceneNode* clone( RenderableEntityManager* renderableEntityManager )
    {
        return new FirstPersonCameraSceneNode( *this );
    }

    static constexpr fnStringHash_t Hashcode = FLAN_STRING_HASH( "FirstPersonCameraSceneNode" );

    FirstPersonCamera* camera;
    bool enabled;

    FirstPersonCameraSceneNode( FirstPersonCamera* nodeCamera, const std::string& name = "FirstPersonCamera" )
        : SceneNode( name )
        , camera( nodeCamera )
        , enabled( false )
    {
    ...
    FirstPersonCameraSceneNode( FirstPersonCameraSceneNode& node )
        : SceneNode( node )
    {
