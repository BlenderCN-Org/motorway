...
#include <Framework/Cameras/LookAtCamera.h>
#include "SceneNode.h"
#include <Graphics/DrawCommandBuilder.h>

struct LookAtCameraSceneNode : public SceneNode
{
    virtual LookAtCameraSceneNode* clone( RenderableEntityManager* renderableEntityManager )
    {
        return new LookAtCameraSceneNode( *this );
    }

    static constexpr fnStringHash_t Hashcode = FLAN_STRING_HASH( "LookAtCameraSceneNode" );

    LookAtCamera* camera;
    bool enabled;

    LookAtCameraSceneNode( LookAtCamera* nodeCamera, const std::string& name = "LookAtCamera" )
        : SceneNode( name )
        , camera( nodeCamera )
        , enabled( false )
    {
    ...
    LookAtCameraSceneNode( LookAtCameraSceneNode& node )
        : SceneNode( node )
    {
