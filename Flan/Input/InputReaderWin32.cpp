...
void flan::core::CreateInputReaderImpl( flan::core::eInputLayout& activeInputLayout )
{
    FLAN_CLOG << "Creating InputReader (WINAPI)" << std::endl;
    ...
    if ( inputDeviceCount == 0 ) {
        FLAN_CERR << "No input device available" << std::endl;
        return;
    }
    ...
    for ( UINT i = 0; i < inputDeviceCount; i++ ) {
        ...
        if ( device.dwType == RIM_TYPEMOUSE ) {
            ...
            fnChar_t inputLayoutName[KL_NAMELENGTH + 1] = { '\0' };
        ...
        //    FLAN_CLOG << "Found unsupported input device with vendorid: " << std::hex << deviceInfos.hid.dwVendorId
    ...
    FLAN_CLOG << "::RegisterRawInputDevices >> operation result : " << ( ( isRegistered == TRUE ) ? "OK" : "FAILED" ) << std::endl;
}
...
void flan::core::ProcessInputEventImpl( InputReader* inputReader, const HWND win, const WPARAM infos, const uint64_t timestamp )
{
    RAWINPUT rawInput = {};
