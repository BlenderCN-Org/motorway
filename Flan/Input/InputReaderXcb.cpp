...
void flan::core::ProcessInputEventImpl( InputReader* inputReader, const xcb_generic_event_t* event )
{
    ...
    case XCB_KEY_RELEASE:{
        ...
        inputReader->pushKeyEvent( { flan::core::UnixKeys[xcb_key_symbols_get_keysym( KEY_SYMBOLS, keyReleaseEvent->detail, 0 )], 0 } );
        break;
    }

    case XCB_KEY_PRESS: {
        const xcb_key_press_event_t* keyPressEvent = reinterpret_cast<const xcb_key_press_event_t*>( event );
        inputReader->pushKeyEvent( { flan::core::UnixKeys[xcb_key_symbols_get_keysym( KEY_SYMBOLS, keyPressEvent->detail, 0 )], 1 } );
        break;
    }
