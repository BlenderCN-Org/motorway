...
TextureCubeArray envProbe : register( t2 );
...
float4 integrateDiffuseCube( in float3 N )
{
    ...
    for ( uint i = 0; i < sampleCount; ++i ) {
        ...
        if ( NdotL > 0 ) {
            accBrdf += envProbe.SampleLevel( PostEffectSampler, float4( L, probeIndex ), 0 ).rgb;
        }
    }
...
#define mipCount 8
            ...
            float4 Li = envProbe.SampleLevel( PostEffectSampler, float4( L, probeIndex ), mipLevel );
