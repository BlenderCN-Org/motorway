...
cbuffer WorldInfos : register( b2 )
{
    float g_TimeDelta;
};    
    
StructuredBuffer<autoExposure_t>	_bufferAutoExposure2 : register( t0 );
...
Buffer<uint>						_texHistogram : register( t1 );		// The image histogram (for debug purpose)
...
void EntryPointCS( uint3 _GroupID : SV_GROUPID, uint3 _GroupThreadID : SV_GROUPTHREADID, uint3 _DispatchThreadID : SV_DISPATCHTHREADID )
{
    ...
    autoExposure_t	LastFrameResult = _bufferAutoExposure2[0];
    ...
    uint	TailBucketPosition = ClippedBucketIndex.x;					// Start from the specified minimum bucket
    uint	HeadBucketPosition = TailBucketPosition;								// We'll make head index grow
    uint	Integral = 0U;
    ...
    if ( LastFrameResult.TargetLuminance < TargetLuminance ) {
        Result.TargetLuminance = lerp( TargetLuminance, LastFrameResult.TargetLuminance, pow( abs( 1.0 - _adapt_speed_up ), g_TimeDelta ) );
    } else {
        ...
        Result.TargetLuminance = 1.0 / lerp( InvTargetLuminanceFactor, InvLastFrameLuminanceFactor, pow( abs( 1.0 - _adapt_speed_down ), g_TimeDelta ) );
    }
