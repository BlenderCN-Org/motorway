        ...
        if ( ImGui::BeginMenu( "File" ) ) {
            if ( ImGui::MenuItem( "New Scene" ) ) {
                *PickedNode = nullptr;
                ...
                g_GameLogic->changeScene( new Scene() );
                auto scene = g_GameLogic->getCurrentScene();

                auto camera = new FreeCamera();
                camera->SetProjectionMatrix( 80.0f, 1920.0f, 1080.0f );

                auto sceneNode = (FreeCameraSceneNode*)scene->createFreeCamera( camera, "DefaultCamera" );
        ...
        if ( ImGui::BeginMenu( "Edit" ) ) {
            ...
            if ( ImGui::MenuItem( "Paste" ) ) {
                if ( CopiedNode != nullptr ) {
                    g_TransactionHandler->commit( new SceneNodeCopyCommand( *PickedNode, g_GameLogic->getCurrentScene(), g_RenderableEntityManager.get(), g_DynamicsWorld.get() ) );
                }
            }

            if ( ImGui::MenuItem( "Delete" ) ) {
                if ( PickedNode != nullptr ) {
                    g_TransactionHandler->commit( new SceneNodeDeleteCommand( *PickedNode, g_GameLogic->getCurrentScene(), g_RenderableEntityManager.get(), g_DynamicsWorld.get() ) );
                    PickedNode = nullptr;
                }
            }
            ImGui::EndMenu();
            ...
            if ( panelId == 0 ) {
                if ( *PickedNode != nullptr ) {
                    ...
                    if ( ImGui::Button( "Delete!" ) ) {
                        ...
                        g_TransactionHandler->commit( new SceneNodeDeleteCommand( node, g_GameLogic->getCurrentScene(), g_RenderableEntityManager.get(), g_DynamicsWorld.get() ) );
                        *PickedNode = nullptr;
                    } else {
                        ImGui::PopStyleColor();
                        ...
                        const auto& sceneNodes = g_GameLogic->getCurrentScene()->getSceneNodes();
