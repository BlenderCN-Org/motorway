...
ResHandle_t AddDownsampleMipRenderPass( RenderPipeline* renderPipeline, ResHandle_t input, const uint32_t inputWidth, const uint32_t inputHeight, const uint32_t downsampleFactor = 2u )
{
    struct PassData
    {
        ResHandle_t input;
    ...
    PassData& passData = renderPipeline->addRenderPass<PassData>(
        ...
        [&]( RenderPipelineBuilder& renderPipelineBuilder, PassData& passData ) {
            //renderPipelineBuilder.setUncullablePass(); // test for debug >> remove it later!
...
ResHandle_t AddBlurPyramidRenderPass( RenderPipeline* renderPipeline, ResHandle_t input, uint32_t inputWidth, uint32_t inputHeight )
{
    ...
    ResHandle_t mip0RenderTarget = AddDownsampleMipRenderPass( renderPipeline, input, inputWidth, inputHeight, 2 );
