...
{
    nyaStringHash_t action;
    nyaStringHash_t state;
...
void InputMapper::addContext( const nyaStringHash_t name, InputContext* context )
{
    inputContexts.emplace( name, context );
}

void InputMapper::pushContext( const nyaStringHash_t name )
{
...
bool InputMapper::mapButtonToAction( eInputKey button, nyaStringHash_t& action ) const
{
    for ( auto iter = activeContexts.begin(); iter != activeContexts.end(); ++iter ) {
        const InputContext* context = *iter;
...
bool InputMapper::mapButtonToState( eInputKey button, nyaStringHash_t& state ) const
{
    for ( auto iter = activeContexts.begin(); iter != activeContexts.end(); ++iter ) {
        const InputContext* context = *iter;
    ...
    while ( file->isGood() ) {
        ...
        if ( keyValueSeparator != nyaString_t::npos ) {
            ...
            if ( !dictionaryValue.empty() ) {
                ...
                switch ( keyHashcode ) {
                case NYA_STRING_HASH( "Context" ): {
                    auto contextHashcode = nya::core::CRC32( nya::core::WrappedStringToString( dictionaryValue ) );
                ...
                default:
                    if ( currentContext == nullptr ) {
                        NYA_CERR << "State " << dictionaryValue << " does not belong to any input context (if this is intended to be a global state, use one of the global context)" << std::endl;
                        break;
                    }

                    if ( dictionaryValue.front() != '{' || dictionaryValue.back() != '}' ) {
                        NYA_CERR << "State " << dictionaryValue << " : invalid syntax (expected opening and closing brackets)" << std::endl;
                        break;
                    }

                    std::size_t buttonTypeEndOffset = dictionaryValue.find_first_of( ',' );
                    nyaString_t buttonTypeString = dictionaryValue.substr( 1, buttonTypeEndOffset - 1 );
                    nya::core::TrimString( buttonTypeString );

                    auto buttonTypeHashcode = nya::core::CRC32( buttonTypeString );
                    ...
                    case eButtonType::ACTION: {
                        ...
                        nyaString_t keyString = dictionaryValue.substr( buttonTypeEndOffset + 1, vecEnd - actionKeyEndOffset - 1 );
                        nya::core::TrimString( keyString );
                        auto inputKeyHashcode = nya::core::CRC32( keyString );
                    ...
                    case eButtonType::AXIS: {
                        ...
                        nyaString_t axisString = dictionaryValue.substr( buttonTypeEndOffset + 1, axisInputEndOffset - buttonTypeEndOffset - 1 );
                        nya::core::TrimString( axisString );
                        auto axisHashcode = nya::core::CRC32( axisString );
                        ...
                        nyaString_t sensitivityRangeString = dictionaryValue.substr( axisInputEndOffset + 1, sensitivityRangeOffset - axisInputEndOffset - 1 );
                        ...
                        nyaString_t inputMinString = dictionaryValue.substr( sensitivityRangeOffset + 1, inputMinOffset - sensitivityRangeOffset - 1 );

                        std::size_t inputMaxOffset = dictionaryValue.find_first_of( ',', inputMinOffset + 1 );
                        nyaString_t inputMaxString = dictionaryValue.substr( inputMinOffset + 1, inputMaxOffset - inputMinOffset - 1 );

                        std::size_t outputMinOffset = dictionaryValue.find_first_of( ',', inputMaxOffset + 1 );
                        nyaString_t outputMinString = dictionaryValue.substr( inputMaxOffset + 1, outputMinOffset - inputMaxOffset - 1 );

                        std::size_t outputMaxOffset = dictionaryValue.find_first_of( ',', outputMinOffset + 1 );
                        nyaString_t outputMaxString = dictionaryValue.substr( outputMinOffset + 1, vecEnd - outputMaxOffset - 1 );
