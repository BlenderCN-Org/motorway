...
static std::size_t BitsPerPixel( const DXGI_FORMAT fmt )
{
    switch ( fmt ) {
...
void nya::core::LoadDirectDrawSurface( FileSystemObject* stream, DirectDrawSurface& data )
{
    uint32_t fileMagic;
    stream->read( fileMagic );

    if ( fileMagic != DDS_MAGIC ) {
        NYA_CERR << "Invalid DDS file!" << std::endl;
        return;
    }
