...
struct Matrix<Precision, 4, 4>
{
    ...
    static constexpr int        SCALAR_COUNT    = ( ROW_COUNT * COLUMN_COUNT );
    ...
    constexpr Vector<Precision, COLUMN_COUNT>& operator[] ( const unsigned int index )
    {
        return rows[index];
    }

    constexpr Precision& operator[] ( const Vector<int, 2> index2d ) const
    {
        return rows[index2d.y][index2d.x];
    }

    constexpr Precision& operator[] ( const Vector<int, 2> index2d )
    {
        return rows[index2d.y][index2d.x];
    }
