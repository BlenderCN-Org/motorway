...
Project Alone Source Code
    ...
    This file is nyart of Project Alone source code.
    ...
    MERCHANTABILITY or FITNESS FOR A nyaRTICULAR PURPOSE.  See the
...
static nyaVec3f TemperatureToRGB( const float temperatureInKelvin )
{
    ...
    return nyaVec3f( R, G, B );
}

static float RGBToTemperature( const float colorRedChannel, const float colorBlueChannel )
{
    ...
    for ( colorTemperature = ( tMax + tMin ) / 2.0f; tMax - tMin > 0.1f; colorTemperature = ( tMax + tMin ) / 2.0f ) {
        nyaVec3f newColor = TemperatureToRGB( colorTemperature );
...
"Tungsten lamp 5K/10K", "Mecury vapor bulb", "Daylight", "RGB Monitor", "Fluorescent lights", "Sky overcast", "Outdoor shade areas", "Sky nyartly cloudy" };
...
static nya::editor::eColorMode activeColorMode = nya::editor::COLOR_MODE_SRGB;

bool nya::editor::PanelLuminousIntensity( float& luminousIntensity )
{
...
bool nya::editor::PanelColor( eColorMode& activeColorMode, nyaVec3f& colorRGB )
{
    bool hasChanged = false;
    ...
    if ( activeColorMode == 0 ) {
        ...
        float colorTemperature = RGBToTemperature( colorRGB[0], colorRGB[2] );
        ...
        colorRGB = TemperatureToRGB( colorTemperature );
    }
