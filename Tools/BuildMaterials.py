    ...
    try:
        ...
        if not ':' in line:
            if 'Layer' in line:
                ...
                material_layer.Displacement = None
        ...
        if 'Name' in key:
            ...
            material_layer.UseBaseColorsRGBSource = read_value_boolean( value )
        elif 'UseAlphaRoughnessSource' in key:
            material_layer.UseAlphaRoughnessSource = read_value_boolean( value )
        elif 'IsNormalMapTangentSpace' in key:
            material_layer.IsNormalMapTangentSpace = read_value_boolean( value )
        elif 'IsSecondaryNormalMapTangentSpace' in key:
            material_layer.IsSecondaryNormalMapTangentSpace = read_value_boolean( value )
        ...
        elif 'NormalMapStrength' in key:
            material_layer.NormalMapStrength = read_value_float( value )
        elif 'SecondaryNormalMapStrength' in key:
            material_layer.SecondaryNormalMapStrength = read_value_float( value )
        elif 'DisplacementMapStrength' in key:
            material_layer.DisplacementMapStrength = read_value_float( value )
    except ValueError:
...
for layer in material_layers:
    layer_build_fun = layer_build_fun + "MaterialReadLayer ReadLayer" + str( layer_id ) + "(in VertexStageData VertexStage, in float3 N, in float3 V, in float3x3 TBNMatrix )"
    layer_build_fun = layer_build_fun + "{ "
    layer_build_fun = layer_build_fun + "MaterialReadLayer layer; "
    
    if type( layer.Displacement ) is str:
        layer_build_fun = layer_build_fun + "float3 viewDirTS = normalize( mul( -V, transpose( TBNMatrix ) ) ); "
        layer_build_fun = layer_build_fun + "float2 uvCoord = ApplyParallaxMapping( VertexStage.uvCoord, V, N, viewDirTS, g_TexDisplacement" + str( layer_id ) + ", " + str( layer.DisplacementMapStrength ) + " );"
    else:
        layer_build_fun = layer_build_fun + "float2 uvCoord = ( VertexStage.uvCoord + float2( " + str( layer.Offset[0] ) + ", " + str( layer.Offset[1] ) + " ) ) * float2( " + str( layer.Scale[0] ) + ", " + str( layer.Scale[1]  ) + " ); "
    ...
    if type( layer.Normal ) is str:
        layer_build_fun = layer_build_fun + "layer.Normal=normalize( layer.Normal * " + layer.NormalMapStrength + " * 2.0f - 1.0f ); "

        if layer.IsNormalMapTangentSpace is True:
    ...
    if type( layer.SecondaryNormal ) is str:
        layer_build_fun = layer_build_fun + "layer.SecondaryNormal=normalize( layer.SecondaryNormal * " + layer.SecondaryNormalMapStrength + " * 2.0f - 1.0f ); "
        
        if layer.IsSecondaryNormalTangentSpace is True:
...
layer_read_fun="MaterialReadLayer BaseLayer=ReadLayer0( VertexStage, N, V, TBNMatrix ); "

for layer in range( 1, layer_id ):
    layer_read_fun = layer_read_fun + "MaterialReadLayer Layer" + str( layer ) + "=ReadLayer" + str( layer ) + "( VertexStage, N, V, TBNMatrix ); "
...
with open( compiled_shader_folder + "/CompilationTest.mat", 'w') as outfile:
